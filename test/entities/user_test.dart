...
void main() {
  group('Teste de entidade "User".', () {
    ...
    test('updateLevel atualiza corretamente o nivel do Usuario', () {
      ...
      user.totalXp =
          375; // XP necessário para o nível 3 é (100 * 1.5 * 3) = 450, então fica no nível 2
      user.updateLevel();
      expect(user.level, 2);
    ...
    test(
      'xpToNextLevel conta corretamente o XP necessario para o proximo nivel',
      () {
        final userLevel1 = User(name: 'Ana', level: 1);
        expect(
          userLevel1.xpToNextLevel(),
          userLevel1.xpForLevel(2) - userLevel1.totalXp,
        ); // Nível 2

        final userLevel3 = User(name: 'Pedro', level: 3);
        expect(
          userLevel3.xpToNextLevel(),
          userLevel3.xpForLevel(4) - userLevel3.totalXp,
        ); // Nível 4
      },
    );

    test(
      'xpForCurrentLevel retorna o XP correto necessario para atingir o nivel atual',
      () {
    ...
    test(
      'xpForNextLevel retorna o XP correto necessario para atingir o proximo nivel',
      () {
    ...
    test(
      'currentLevelXp retorna o XP correto acumulado dentro do nivel atual',
      () {
    ...
    test(
      'requiredXpForCurrentLevel retorna o XP correto necessario para atingir o proximo nivel',
      () {
